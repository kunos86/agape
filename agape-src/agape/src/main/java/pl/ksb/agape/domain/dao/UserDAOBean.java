...
public class UserDAOBean  {
	...
	public boolean isRegistered(String mail) {
		...
		List<User> osoby = em.createNamedQuery("getUserByMail").setParameter("email", mail).getResultList();
	...
	public List<User> getUsers(int firstRow, int numRows){
		return em.createNamedQuery("getUsers").setFirstResult(firstRow).setMaxResults(numRows).getResultList();	
	}
	...
	public List<User> getStudentsWithoutTeacher(){
		...
		return em.createQuery("select u from User u where not exists (Select id from StudentTeacher s where s.student.id = u.id and s.current = 'T') "
				+ "and exists (select r.id from Role r where r.user.id=u.id and r.roleName='STUDENT')").getResultList();
	}
	...
	public Long getCountStudentsWithoutTeacher(){
		...
		return (Long)em.createQuery("select count(u) from User u where not exists(Select id from StudentTeacher s where s.student.id = u.id and s.current = 'T') "
				+ "and exists (select r.id from Role r where r.user.id=u.id and r.roleName='STUDENT')").getSingleResult();
	}
	
	@SuppressWarnings("unchecked")
	public List<User> getTeachers(){
		return em.createQuery("select u from User u where exists (Select id from UczenNauczyciel st where st.uczen.id = u.id and st.aktualny = 'T') ").getResultList();
